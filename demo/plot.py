...
def run_simulation_with_plots():
    ...
    T = p["T"]
    dt = p["dt"]
    PlotPeriod = p["PlotPeriod"]

    maxiterMicro = int(np.ceil(T / dt))
    maxiterPlot = int(np.ceil(maxiterMicro / PlotPeriod))
    ...
    for iterMicro in range(1, maxiterMicro + 1):
        ...
        if p.get("FLcoupling", 0) and (iterMicro % p["CouplingPeriod"] == 0):
            permissivity = sarcomere.compute_permissivity()
            Pfrac = permissivity[0]
            gammaF += (
                dt * p["CouplingPeriod"] / p["muA"] * (-p["alpha"] * Pfrac + (1 / (1 + gammaF) - 1))
            )
            SL_t = p["l0"] * (1 + gammaF)
        elif not p.get("FLcoupling", 0):
        ...
        TimeODE += time.time() - t0

        # 2d. Plotting
        if iterMicro % PlotPeriod == 0:
            # Stats for plotting
            ...
            axs[0].clear()
            axs[0].plot(tt, [calcium_trace(x) for x in tt])
            ...
            t_ode_avg = TimeODE / iterMicro * 1000
            fig.suptitle(f"ODE={t_ode_avg:.2f}ms")

            plt.pause(0.001)
            iterPlot += 1

    print(f"Total Simulation Time: {time.time() - TimeInit:.2f}s")
    plt.ioff()
    plt.show()
